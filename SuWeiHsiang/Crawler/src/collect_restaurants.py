...
def get_abs_locate(path,night_market_name,dir):#加上經緯度單函式
    df = pd.read_csv(path)
    ...
    for i in range(df.shape[0]):
        ...
        time.sleep(3)#等待時間可以調高，或是可以有更好的寫法，目標是讓絕對位置完全呈現
...
def get_restaurant_locate(path,dir):
    df = pd.read_csv(path)
    ...
    for i in range(df.shape[0]):
        ...
        try:
            ...
            site.append(driver.find_element(By.CLASS_NAME,"CsEnBe").get_attribute("aria-label").lstrip("地址: "))
            driver.close()
        except:
            site.append(" ")
            print(f"爬取{df.iloc[i,0]}時發生錯誤")

    df.insert(1,"地址",site)
    path = Path(f"./data/{dir}")
    path.mkdir(parents=True,exist_ok=True)
    df.to_csv(f"{path}/restaurant_around_corner_withsite.csv",header=True,index=False,encoding='utf-8-sig')


if __name__ == "__main__" :
    ###########以下請設定###########
    chinese_nightmarket = "樂華夜市"   #店家的中文名稱(作為Google地圖搜尋關鍵字)
    typename = "小吃"                 #店家的類型(如小吃、餐廳等等)
    nightmarket = "Lehua"             #店家的英文名稱(為了創建檔案存取資料夾用)
    target_latitude = 25.0085602      #店家的緯度
    target_longitude = 121.5102631    #店家的經度
    ...
    get_restaurant_locate(f"./data/{nightmarket}/restaurant_around_corner.csv",dir)