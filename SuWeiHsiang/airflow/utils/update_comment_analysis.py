    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "你是一位專業的評論分析師，專門用於判斷餐廳評論是否疑似為水軍留言。"
                    "對每則評論資訊請根據以下規則進行判斷：\n\n"
                    "1. 回答 T（是水軍）或 F（非水軍）。\n"
        ...
        print(
            f"[Token 使用狀況] input: {usage.prompt_tokens}｜output: {usage.completion_tokens}｜total: {usage.total_tokens}"
        )
...
def output_csv(df, df_group, output_text):
    ...
    if len(lines) != len(df_group):
        print(
            f"回傳筆數與預期不一致：預期 {len(df_group)}，實際 {len(lines)}\n內容如下：\n{output_text}"
        )
        return
...
def main():
    ...
    df_output.to_csv(
        "./data/comment_analysis_result.csv",
        index=False,
        header=True,
        encoding="utf-8-sig",
    )
